...
% function generaGraficas(Puntos3D,ct,Puntos2Dt,v_tiempo,v_posiciones,v_correccionesvw,v_pts,v_HF)
%A partir de la simulaci�n genera todo gr�ficas con los resultados
...
grid;
legend('x','y','z');
title('Evoluci�n de posiciones (x y z)');

subplot(2,3,4);
plot(v_tiempo,v_posiciones(:,4:6));  
grid;
legend('Rx','Ry','Rz');
title('Evoluci�n de orientaciones (Rx Ry Rz)');
...
grid;
legend('wx','wy','wz');
title('Velocidades de rotaci�n (wx wy wz)');
...
if nargin>7    
    ...
    if rango==3
        %H
        figure
        subplot(3,3,1); plot(v_tiempo, v_HF(:,1)); title('H_{11}');title('Evoluci�n homograf�as');grid;
        ...
        %F
        figure
        subplot(3,3,1); plot(v_HF(:,1)); title('F_{11}');title('Evoluci�n F');grid;
